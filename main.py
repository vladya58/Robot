...
last_index_rasschet_min = len(spisok) + 12
last_value_rasschet_min = raschetmin(last_value_rasschet_min, koef, len(spisok) - luch_min + 12, ostatok)

luch_max = last_index_rasschet_max
last_index_rasschet_max = len(spisok) + 12
last_value_rasschet_max = raschetmax(last_value_rasschet_max, koef, len(spisok) - luch_max + 12, ostatok)
...
print(f"Точка через которую пойдет нижняя параллель по О.х [{len(spisok)-1-I_down}]",f"Точка через которую пойдет нижняя параллель по О.y [{V_down}]")
